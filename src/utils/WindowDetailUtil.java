	...
	public static List<WindowInfo> getWindows() {
		...
		user32.EnumWindows((hWnd, data) -> {
			try {
				...
				Vector2 dpiVec = getDPI();
				int left = (int) Math.round(rect.left / dpiVec.y);
				int top = (int) Math.round(rect.top / dpiVec.y);
				int right = (int) Math.round(rect.right / dpiVec.y);
				int bottom = (int) Math.round(rect.bottom / dpiVec.y);
				...
				list.add(new WindowInfo(hWnd, scaledRect, title, scale));
			} catch (Throwable e) {
